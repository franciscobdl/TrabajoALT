...
class SpellSuggester:
    ...
    def suggest(self, term, distance=None, threshold=None, flatten=True):
        ...
        """
        if distance is None:    # si no se especifica, se usa el por defecto    
            distance = self.default_distance 
        if threshold is None:   
            threshold = self.default_threshold
            
        fdist = self.distance_functions[distance]   # función de distancia

        resul = [[] for i in range(threshold+1)] # hasta threshold inclusive
        for w in self.vocabulary:   # para cada palabra del vocabulario
            d = fdist(term,w,threshold) # distancia entre el término y la palabra del vocabulario
            if d <= threshold: # si la distancia es menor o igual que el threshold
                resul[d].append(w)  
